  ...
  module MapperExtension
    ...
    def build_resources(*default_options)
      ...
      def resources_mappings(map, dir, conditions={}, only=nil)
        mappings = []
        Dir["#{RAILS_ROOT}/app/controllers/#{dir}/*_controller.rb"].each do |file|
          controller = eval(file[file.index("/app/controllers/") + "/app/controllers".length + 1 .. file.length - ".rb".length - 1].classify)
