  ...
  module PlResources
    class PermalinkResource #:nodoc:
      ...
      def initialize(entities, options)
        @plural   ||= entities
        options[:path] ||= entities.to_s
        options[:member_path] = entities.to_s.singularize if options[:member_path].nil?
      ...
      def path
        options[:path].empty? ? "#{path_prefix}" : "#{path_prefix}/" + (options[:path])
      end

      def new_path
        @new_path ||= "#{inner_path}/" + (options[:new_path] || "new")
      end

      def member_path
        @member_path ||= "#{inner_path}/:id"
      end

      def nesting_path_prefix
        @nesting_path_prefix ||= "#{inner_path}/:#{options[:model] || singular + '_id'}"
      end
      ...
      protected
        def inner_path
          @path ||= options[:member_path].empty? ? "#{path_prefix}" : "#{path_prefix}/#{options[:member_path]}"
        end
    ...
    class SingletonPermalinkResource < PermalinkResource #:nodoc:
      def initialize(entity, options)
        @singular = @plural = entity
        options[:controller] ||= @singular.to_s.pluralize
        options[:path] ||= entity.to_s
        options[:member_path] = options[:path]
        @options = options
    ...
    def pl_resources(*entities, &block)
      options = entities.extract_options!
      entities.each { |entity| map_permalinked_resource(entity, options.dup, &block) }
    end
    ...
    def pl_resource(*entities, &block)
      options = entities.extract_options!
      entities.each { |entity| map_permalinked_singleton_resource(entity, options.dup, &block) }
    end

    private
      def map_permalinked_resource(entities, options = {}, &block)
        resource = PermalinkResource.new(entities, options)

        with_options :controller => resource.controller do |map|
          pl_map_collection_actions(map, resource)
          pl_map_default_collection_actions(map, resource)
          pl_map_new_actions(map, resource)
          pl_map_member_actions(map, resource)
      ...
      def map_permalinked_singleton_resource(entities, options = {}, &block)
        resource = SingletonPermalinkResource.new(entities, options)

        with_options :controller => resource.controller do |map|
          pl_map_collection_actions(map, resource)
          pl_map_default_singleton_actions(map, resource)
          pl_map_new_actions(map, resource)
          pl_map_member_actions(map, resource)
      ...
      def pl_map_collection_actions(map, resource)
      ...
      def pl_map_default_collection_actions(map, resource)
        ...
        unless resource.options[:only]
          create_action_options = action_options_for("create", resource)
          map.connect(resource.path, create_action_options)
          map.connect("#{resource.path}.:format", create_action_options)
        else
          create_action_options = action_options_for("create", resource)
          map.connect("#{resource.path}/create", create_action_options)
          map.connect("#{resource.path}.:format/create", create_action_options)
        end
      end

      def pl_map_default_singleton_actions(map, resource)
        unless resource.options[:only]
          create_action_options = action_options_for("create", resource)
          map.connect(resource.path, create_action_options)
          map.connect("#{resource.path}.:format", create_action_options)
      ...
      def pl_map_new_actions(map, resource)
      ...
      def pl_map_member_actions(map, resource)
        resource.member_methods.each do |method, actions|
          actions.each do |action|
            action_options = pl_action_options_for(action, resource, method)
        ...
        show_action_options = pl_action_options_for("show", resource)
        map.named_route("#{resource.name_prefix}#{resource.singular}", resource.member_path, show_action_options)
        map.named_route("formatted_#{resource.name_prefix}#{resource.singular}", "#{resource.member_path}.:format", show_action_options)

        unless resource.options[:only]
          update_action_options = pl_action_options_for("update", resource)
          map.connect(resource.member_path, update_action_options)
          map.connect("#{resource.member_path}.:format", update_action_options)

          destroy_action_options = pl_action_options_for("destroy", resource)
          map.connect(resource.member_path, destroy_action_options)
          map.connect("#{resource.member_path}.:format", destroy_action_options)
        else
          update_action_options = pl_action_options_for("update", resource)
          map.connect("#{resource.member_path}/update", update_action_options)
          map.connect("#{resource.member_path}.:format/update", update_action_options)

          destroy_action_options = pl_action_options_for("destroy", resource)
          map.connect("#{resource.member_path}/destroy", destroy_action_options)
          map.connect("#{resource.member_path}.:format/destroy", destroy_action_options)
        end
      end
      ...
      def pl_action_options_for(action, resource, method = nil)
        default_options = { :action => action.to_s }
        require_id = !resource.kind_of?(SingletonPermalinkResource)
        unless resource.options[:only]
          ...
          default_method = resource.options[:only]
          case default_options[:action]
            when "index", "new"; default_options.merge(add_conditions_for(resource.conditions, method || default_method)).merge(resource.requirements)
            when "create";       default_options.merge(add_conditions_for(resource.conditions, method || default_method)).merge(resource.requirements)
            when "show", "edit"; default_options.merge(add_conditions_for(resource.conditions, method || default_method)).merge(resource.requirements(require_id))
            when "update";       default_options.merge(add_conditions_for(resource.conditions, method || default_method)).merge(resource.requirements(require_id))
            when "destroy";      default_options.merge(add_conditions_for(resource.conditions, method || default_method)).merge(resource.requirements(require_id))
            else                 default_options.merge(add_conditions_for(resource.conditions, method)).merge(resource.requirements)
          end
        end
      end
...
class ActionController::Routing::RouteSet::Mapper
  include ActionController::PlResources
end
